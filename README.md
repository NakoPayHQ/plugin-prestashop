...
## Differences vs. the Blockonomics reference

| Concern | Blockonomics | NakoPay |
|---|---|---|
| Webhook auth | `?secret=` query param | `X-NakoPay-Signature` HMAC-SHA256 |
| Front JS | AngularJS + jQuery + Bootstrap UI kit (~250 KB) | Vanilla JS + `qrious` (~10 KB) |
| Polling | 60s | 5s |
| Multi-coin UI | per-coin sub-domains | single-coin v0.1, scaffolded for v0.2 |
| API base | hardcoded | dual-base-URL with admin override + `NAKOPAY_API_BASE` constant |
